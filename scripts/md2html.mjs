// README(한/영) 2개 마크다운을 각 문서와 같은 내용의 HTML로 변환합니다.
// 사용법: app에서 npm ci를 마친 뒤 프로젝트 루트에서 node scripts/md2html.mjs를 실행합니다.
// marked는 실행 앱이 아닌 문서 생성 전용 devDependency입니다.
//
...
function wrap(title, bodyHtml, lang) {
  return `<!doctype html>
<html lang="${lang}">
<head>
<meta charset="utf-8">
...
const files = [
  { md: 'README.md', html: 'README.html', title: '소담 오브레인 Codex — README', lang: 'ko' },
  { md: 'README.en.md', html: 'README.en.html', title: 'SoDam O-Brain for Codex — README', lang: 'en' },
];
...
for (const f of files) {
  ...
  const finalHtml = wrap(f.title, bodyHtml, f.lang);
